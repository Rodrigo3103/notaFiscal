...
class Ui_formCliente(object):
    def setupUi(self, formCliente):
        ...
        self.bt_alterarDados.clicked.connect(self.frameDadosCliente.hide) # type: ignore
    ...
    def retranslateUi(self, formCliente):
        ...
        self.bt_alterarDados.setText(_translate("formCliente", "Confirmar"))
        ...
        self.bt_consultar.clicked.connect(self.consultarcliente)  # Todo: consulta o forncecedor.
        self.bt_alterar.clicked.connect(self.abrirDadosclientes)  # Todo: carrega os dados dos clientes na tela.
        self.bt_alterarDados.clicked.connect(
            self.alterarDadoscliente)  # Todo: faz a alteração dos dados clientees.
        self.bt_excluir.clicked.connect(self.excluircliente)  # Todo: exclui o cliente.
        self.consultarGeral()  # Todo carrega a tabela.
        self.tb_cliente.selectRow(0)  # Todo: aqui pega-se a primeira linha da tabela.
        ...
        self.bt_incluir.clicked.connect(self.incluircliente)  # Todo: abrr a tela incluir cliente
    ...
    def consultarGeral(self):
        ...
        try:
            ...
            df_cliente = pd.DataFrame(myresult,
                                         columns=['ID cliente', 'RAZÃO SOCIAL', 'NOME FANTASIA', 'CLASSIFICAÇÃO',
                                                  'CNPJ CPF',
            ...
            # self.tb_cliente.setHorizontalHeaderLabels(self.all_data.columns) #Todo:Setar o título e cabeçalho da tabela
    ...
    def consultarcliente(self):
        ...
        mycursor = mydb.cursor()
        consultaConsulta = f'SELECT * FROM cliente WHERE idcliente= "{variaveisDeControles.idConsulta}"'
        ...
        mycursor.close()
        df_cliente = pd.DataFrame(myresult,
                                     columns=['idcliente', 'razaoSocial', 'nomeFantasia', 'classificacao', 'cnpjcpf',
                                              'ierg', 'endereco', 'numero', 'bairro', 'cep', 'cidade', 'estado',
                                              'telefone', 'email', 'observacao', 'prazo', 'usuarioCadastro',
                                              'auditoria'])

        self.lb_razaoSocial.setText(df_cliente.loc[0][1])
        self.lb_nomeFantasia.setText(df_cliente.loc[0][2])
        self.lb_class.setText(df_cliente.loc[0][3])
        self.lb_cnpjcpf.setText(df_cliente.loc[0][4])
        self.lb_ierg.setText(df_cliente.loc[0][5])
        self.lb_endereco.setText(df_cliente.loc[0][6])
        self.lb_numero.setText((str(df_cliente.loc[0][7])))
        self.lb_bairro.setText(df_cliente.loc[0][8])
        self.lb_cep.setText(df_cliente.loc[0][9])
        self.lb_uf.setText(df_cliente.loc[0][10])
        self.lb_cidade.setText(df_cliente.loc[0][11])
        self.lb_telefone.setText(df_cliente.loc[0][12])
        self.lb_email.setText(df_cliente.loc[0][13])
        self.lb_observacao.insertPlainText(df_cliente.loc[0][14])
        self.lb_prazo.setText(df_cliente.loc[0][15])
        self.frameTab1.setEnabled(False)
        self.frameTab2.setEnabled(False)

    def abrirDadosclientes(self):
        ...
        sql = f"SELECT * FROM cliente WHERE idcliente = '{variaveisDeControles.idConsulta}'"
        mycursor.execute(sql)
    ...
    def alterarDadoscliente(self):
        ...
        if self.tb_cliente.rowCount() > 1:

            print('Alterando')
            linha = self.tb_cliente.currentRow()
            idclienteLinha = self.tb_cliente.item(linha, 0).text()
            print(f"Linha: {linha}, ID cliente: {idclienteLinha}")
            # Todo: seta labels das variaveis.
            razaoSocial = self.lb_razaoSocial.text()  # 1
            nomeFantasia = self.lb_nomeFantasia.text()  # 2
            classificacao = self.lb_class.text()  # 3
            ...
            try:
                ...
                sql = f"UPDATE cliente SET razaoSocial = '{razaoSocial}', nomeFantasia = '{nomeFantasia}', classificacao = '{classificacao}', cnpjcpf = '{cnpjcpf}', ierg = '{ierg}', endereco = '{endereco}', numero = '{numero}', bairro = '{bairro}', cep = '{cep}', cidade = '{cidade}', estado = '{estado}', telefone = '{telefone}', email = '{email}', observacao = '{observacao}', prazo = '{prazo}' WHERE idcliente = '{idclienteLinha}'"
                ...
                msg.setText(f'ID: {idclienteLinha} alterado com sucesso')
                msg.exec()
                self.consultarGeral()
                # self.limpaTela()
                ...
                print(f'\033[0;34m{idclienteLinha}\033[m')
                print('Id de alteração: ', idclienteLinha)
            except:
                print('')
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Critical)
    ...
    def formatoCadastro(self):
        ...
        self.lb_razaoSocial.setStyleSheet(
            'background-color: rgb(255, 255, 255); color: rgb(0, 170, 0); font: 10pt "Verdana";')
        self.txt_razaoSocial.setStyleSheet('font: 87 10pt "Arial";')

        self.lb_nomeFantasia.setStyleSheet(
            'background-color: rgb(255, 255, 255); color: rgb(0, 170, 0); font: 10pt "Verdana";')
        self.txt_nomeFantasia.setStyleSheet('font: 87 10pt "Arial";')

        self.lb_class.setStyleSheet(
            'background-color: rgb(255, 255, 255); color: rgb(0, 170, 0); font: 10pt "Verdana";')
        self.txt_class.setStyleSheet('font: 87 10pt "Arial";')

        self.lb_cnpjcpf.setStyleSheet(
            'background-color: rgb(255, 255, 255); color: rgb(0, 170, 0); font: 10pt "Verdana";')
